...
print("DEBUG ENV:", os.getenv("DB_PASSWORD"))

# 🔥 Only added this for Firefox compatibility
...
def history():
    ...
    cursor.execute(
        "SELECT product_name, search_time FROM products WHERE user_id=%s ORDER BY search_time DESC",
        (session['user_id'],)
    )
    history = cursor.fetchall()
    return render_template('history.html', history=history)
...
def get_avg_price_chart(df):
    ...
    fig = go.Figure(go.Bar(
        x=avg_data["Website"],
        y=avg_data["Price"],
        marker=dict(color=['#FF6A00', '#713FFF', '#2BBF0A', '#00A8E8'])
    ))
...
def dashboard():
    ...
    try:
        ...
        return render_template("dashboard.html", error="No scraped data found.")

    df["Price"] = pd.to_numeric(df["Price"], errors="coerce")
    df = df.dropna(subset=["Price"])

    # charts
    graph_html = get_avg_price_chart(df)
    histogram_html = get_histogram(df)
    pie_html = get_pie_chart(df)

    cheap = df.nsmallest(10, "Price").to_dict("records")
    expensive = df.nlargest(10, "Price").to_dict("records")
