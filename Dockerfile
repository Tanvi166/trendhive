...
# Install essential tools & Chrome dependencies
...
RUN CHROME_VERSION=$(google-chrome --version | sed 's/Google Chrome //') \
    ...
    && wget -q -O /tmp/chromedriver.zip "https://chromedriver.storage.googleapis.com/$(google-chrome --version | sed 's/Google Chrome //')/chromedriver_linux64.zip" || true \
